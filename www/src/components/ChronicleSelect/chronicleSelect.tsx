...
function ChronicleSelect({ submitFunction }: Props) {
    ...
    const isIncluded = (id: number) =>
        selectedChronicles.some(e => e.id === id);
    ...
    return (
        <div className="monolithium-border ">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4 m-2 h-[400px] overflow-scroll
">
                {!chronicles ? (
                    <div>Loading...</div>
                ) : (
                    chronicles.map(element => {
                        ...
                        return (
                            <div key={element.id} className={cardClasses}>
                                <Button onClick={() => handleClick(element.id)} className="w-full h-full">
                                    <div className="font-bold text-md mb-2 rounded-20 bg-gray-300 text-center">
                                        {element.title}
                                    </div>
                                    <div className="h-full flex flex-col space-y-3">
                                        <div>Zeitraum: <br />{element.knots?.toString()}</div>
                                        <div>Category: <br /> {element.category}</div>
                                        <div >Description: <br /> {element.description.length > 60 ?
                                            `${element.description.substring(0, 60).trim()}...` : element.description
                                        }</div>
                                    </div>
                                </Button>
                            </div>
            ...
            <div className="h-[100px] flex items-center justify-center">
                <Button className="h-20 px-4" onClick={() => submitFunction(selectedChronicles)}>
                    Submit
                </Button>
            </div>
        </div >
