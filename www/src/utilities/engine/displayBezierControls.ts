...
import shortenLine from "@/utilities/engine/shortenLine";
...
const displayBezierControls = (path: Path, options?: { shorten: number }) => {
  path.vertices.forEach((anchor, index) => {
    ...
    const rightHandle = new Two.Line(anchor.x, anchor.y, endVectors.right.x, endVectors.right.y);
    rightHandle.stroke = "#00ff";
    if (options && options.shorten) shortenLine(rightHandle, options.shorten);

    const rightHandleDot = new Two.Circle(rightHandle.vertices[1].x, rightHandle.vertices[1].y, 2);
