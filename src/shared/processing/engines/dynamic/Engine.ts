...
class Engine extends Butterfly<EngineChronicle> {
  ...
  private emitDebug(kind: "debug" | "warn" | "group", payload: unknown) {
    if (!logging) return;

    let chain = logging.log("vita")("engine")(kind);
    for (const segment of this.debugGroupStack) {
      chain = chain(segment);
    }
  ...
  private debugLog(...args: any[]) {
    this.emitDebug("debug", args.length <= 1 ? args[0] : args);
  }

  private debugWarn(...args: any[]) {
    this.emitDebug("warn", args.length <= 1 ? args[0] : args);
  }

  private debugGroup(...args: any[]) {
    const label = args.map(String).join(" ");
    this.debugGroupStack.push(label);
    this.emitDebug("group", {
      open: label,
      depth: this.debugGroupStack.length,
    });
  }

  private debugGroupEnd() {
    const closed = this.debugGroupStack.pop();
    if (!closed) return;
    this.emitDebug("group", {
      close: closed,
      depth: this.debugGroupStack.length,
  ...
  private getInsertionDepth(
    ...
    const lastCellByY = new Map(lastCells.map(cell => [cell.y, cell]));

    const getNextX = (y: number) => {
      const lastCell = lastCellByY.get(y);
      return lastCell ? lastCell.x + 1 : 0;
    };

    const hasSpaceAtY = (y: number) => {
      const lastCell = lastCellByY.get(y);
      if (!lastCell) return true;
      return getEngineChronicleLeftDelta(insertionChronicle, lastCell.$) <= 0;
    };

    const getLeftDeltaAtY = (y: number) => {
      const lastCell = lastCellByY.get(y);
      if (!lastCell) return null;
      return getEngineChronicleLeftDelta(insertionChronicle, lastCell.$);
    };
    ...
    const maxAbsDepth = lastCells.reduce(
      (max, cell) => Math.max(max, Math.abs(cell.y)),
      0,
    );

    const positiveDurationWeight =
      this.getDurationWeightByPolarity(BipolarLinkedListPolartity.POSITIVE) ??
      0;
    const negativeDurationWeight =
      this.getDurationWeightByPolarity(BipolarLinkedListPolartity.NEGATIVE) ??
      0;

    this.debugLog("insertion sweep setup", {
      chronicleId: insertionChronicle.id,
      maxAbsDepth,
      positiveDurationWeight,
      negativeDurationWeight,
      positiveLevels: this.getAllPositive().map(v => v.index),
      negativeLevels: this.getAllNegative().map(v => v.index),
    });

    // Neutral depth has highest priority.
    if (hasSpaceAtY(0)) {
      const result = {
        y: 0,
        x: getNextX(0),
      };
      this.debugLog("insertion decision", {
        strategy: "neutral",
        result,
        leftDelta: getLeftDeltaAtY(0),
      });
      this.debugGroupEnd();
      return result;
    }

    this.debugLog("neutral blocked", {
      leftDelta: getLeftDeltaAtY(0),
    });

    // Then check each absolute depth pair (+d, -d), including missing levels.
    for (let absDepth = 1; absDepth <= maxAbsDepth; absDepth++) {
      const posY = absDepth;
      const negY = -absDepth;

      const hasPositiveSpace = hasSpaceAtY(posY);
      const hasNegativeSpace = hasSpaceAtY(negY);

      this.debugLog("depth candidate", {
        absDepth,
        posY,
        negY,
        hasPositiveSpace,
        hasNegativeSpace,
        positiveLeftDelta: getLeftDeltaAtY(posY),
        negativeLeftDelta: getLeftDeltaAtY(negY),
        positiveNextX: getNextX(posY),
        negativeNextX: getNextX(negY),
      });

      if (!hasPositiveSpace && !hasNegativeSpace) continue;

      if (hasPositiveSpace && hasNegativeSpace) {
        const targetY =
          positiveDurationWeight > negativeDurationWeight ? negY : posY;
    ...
    if (positiveDurationWeight > negativeDurationWeight) {
      ...
      this.debugLog("insertion decision", {
        strategy: "fallback-new-negative",
        result,
      });
      this.debugGroupEnd();
      return result;
    ...
    if (positiveDurationWeight < negativeDurationWeight) {
      ...
      this.debugLog("insertion decision", {
        strategy: "fallback-new-positive",
        result,
      });
      this.debugGroupEnd();
      return result;
    ...
    this.debugLog("insertion decision", {
      strategy: "fallback-new-positive-tie",
      result,
    });
    this.debugGroupEnd();
    return result;
      ...
      this.reduceYToNeutral(
        ...
        (accumulator, level, cy) => {
          ...
          if (last) {
            ...
            /** Correct previous projection slice end before adding a new one */
            if (accumulator.length > 0) {
              accumulator[accumulator.length - 1].knots.end = last.$.knots.end;
            }
  ...
  private getTakeoverPath(
    ...
    this.debugLog("takeover input", {
      insertionDepth,
      chronicleKnots: insertionChronicle.knots,
    });

    /** Find `projection` */
    const rawProjection = this.getProjection(
      insertionChronicle,
      insertionDepth,
    );
    const degenerateSlices = rawProjection.filter(
      slice => slice.knots.end <= slice.knots.start,
    );
    const projection = rawProjection.filter(
      slice => slice.knots.end > slice.knots.start,
    );

    if (degenerateSlices.length > 0) {
      this.debugWarn("degenerate projection slices skipped", {
        chronicleId: insertionChronicle.id,
        insertionDepth,
        insertionChronicleKnots: insertionChronicle.knots,
        degenerateSlices,
      });
    }
    ...
    this.debugLog(
      ...
      projection.map(v => {
        ...
        const end = new Date(v.knots.end).toISOString();

        return { ...v, knots: { start, end } };
      }),
    );
    this.debugLog("projection summary", {
      rawProjectionCount: rawProjection.length,
      filteredProjectionCount: projection.length,
      takeoverThreshold: Engine.MANEUVER_TAKEOVER_SPACE_MIN,
    });
    ...
    if (!projection.length) {
      ...
      this.debugLog("takeover decision", {
        strategy: "no-projection-direct-insert",
        target: insertionDepth,
      });
      this.debugGroupEnd();
      return;
    }
    ...
    const takeoverSlices = projection.filter(slice => {
      const duration = slice.knots.end - slice.knots.start;
      return duration >= Engine.MANEUVER_TAKEOVER_SPACE_MIN;
    });

    // Keep chain direction monotonic in time to avoid backward connection curves.
    takeoverSlices.sort((a, b) => a.knots.start - b.knots.start);

    this.debugLog("takeover slices", takeoverSlices);

    // If projection exists but no slice is large enough for a takeover, keep the
    // original straight placement instead of creating a partial/empty takeover path.
    if (takeoverSlices.length === 0) {
      this.set(insertionDepth.y, insertionDepth.x, insertionChronicle);
      this.debugLog("takeover decision", {
        strategy: "projection-without-takeover-direct-insert",
        target: insertionDepth,
      });
      this.debugGroupEnd();
      return;
    ...
    const rootStart = insertionChronicle.knots.start;
    const rootEnd = takeoverSlices[0]?.knots.start;
    const hasRootSegment =
      typeof rootEnd === "number" &&
      Number.isFinite(rootEnd) &&
      rootEnd > rootStart;

    if (!hasRootSegment && takeoverSlices.length > 0) {
      this.debugWarn("root segment skipped", {
        chronicleId: insertionChronicle.id,
        insertionDepth,
        rootStart,
        rootEnd,
    ...
    const result = hasRootSegment
      ? this.set(insertionDepth.y, insertionDepth.x, {
          ...insertionChronicle,
          knots: {
            start: rootStart,
            end: rootEnd,
    ...
    takeoverSlices.forEach(slice => {
      ...
      this.debugLog("takeover segment inserted", {
        targetY: slice.y,
        insertedX: result.x,
        knots: slice.knots,
      });
  ...
  public get positiveScore(): number {
    return (
      this.getDurationWeightByPolarity(BipolarLinkedListPolartity.POSITIVE) ?? 0
    );
  }

  public get negativeScore(): number {
    return (
      this.getDurationWeightByPolarity(BipolarLinkedListPolartity.NEGATIVE) ?? 0
    );
  }
  ...
  private getDurationWeightByLevel(y: number): number | undefined {
    ...
    if (level) {
      ...
      for (const { cell } of level) {
        const duration = cell.$.knots.end - cell.$.knots.start;
  ...
  private getDurationWeightByPolarity(polarity: BipolarLinkedListPolartity) {
    switch (polarity) {
      case BipolarLinkedListPolartity.POSITIVE:
        return this.getAllPositive().reduce((acc, layer) => {
          const dw = this.getDurationWeightByLevel(layer.index);
          if (dw !== undefined) return acc + dw;
          return acc;
        }, 0);
      case BipolarLinkedListPolartity.NEGATIVE:
        return this.getAllNegative().reduce((acc, layer) => {
          const dw = this.getDurationWeightByLevel(layer.index);
          if (dw !== undefined) return acc + dw;
          return acc;
        }, 0);
