...
export const sqlToZodMap = {
  ...
  _json: z.array(z.unknown()), // Fuer JSON Arrays
  _date: z.array(z.string().regex(/^\d{4}-\d{2}-\d{2}$/)),
  _boolean: z.array(z.boolean()), // Fuer Boolean Arrays
  ...
  switch (sqlType) {
    ...
    case "numeric":
      if (numericPrecision && numericScale) {
        // Beispiel: Praezision und Skalierung fuer numerische Werte anwenden
        schema = (schema as z.ZodString).refine((val: string) => {
          const maxValue = Math.pow(10, numericPrecision - numericScale) - 1; // Fuer die genauen Werte der Praezision
          return parseFloat(val) <= maxValue;
        }, "Wert ueberschreitet die erlaubte Praezision und Skalierung.");
      }
      break;
  }
