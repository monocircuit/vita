...
import { sqlToZod, sqlToZodMap } from "@/vendor/utilities/functions";
...
/**
 * Internal schema builder that converts a table definition into a zod schema object.
 *
 * Output structure (per table):
 * - `Raw`: exact DB row shape
 * - `Normalize`: DB row -> app-level normalized shape
 * - `Denormalize`: normalized shape -> DB row payload
 * - `Mutations`: optional projection/reversal schemas for custom views
 */
...
export async function createZodSchemaFromTable<
  ...
  Raw extends object = DbRow<DB, Table>,
...
>(
  ...
  const normalizeSchema = rawSchema.transform((value: any) => {
    const camel = keysToCamelCase(value as any) as KeysToCamelCase<Raw>;
    ...
    for (const key of keys) {
      ...
      transformed[key as string] = normalize(
        camel[key as keyof KeysToCamelCase<Raw>] as any,
      );
    }
